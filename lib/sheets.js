...
async function appendTransaction({ expenseType, category, amount, merchant = '', notes = '', source, rawInput = '', rate = '', quantity = '', finalBill = '', receiptDate = '', monthTab = '', isLastItemOfReceipt = false }) {
  return callProxy('appendTransaction', { expenseType, category, amount, merchant, notes, source, rawInput, rate, quantity, finalBill, receiptDate, monthTab, isLastItemOfReceipt });
}
...
async function appendRawOcr(receiptId, rawText) {
  return callProxy('appendRawOcr', { receiptId, rawText });
}
