...
function classify(rawCategoryText) {
  ...
  for (const rule of CATEGORY_MAP) {
    // Word-boundary match, not plain substring — otherwise "cab" (taxi)
    // would false-positive match inside "cabbage", "bus" inside "business", etc.
    if (rule.keywords.some((kw) => new RegExp(`\\b${kw.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i').test(text))) {
