...
async function handleText(message) {
  ...
  if (text.startsWith('/undo')) {
    ...
    return telegram.sendMessage(chatId, `Removed: ${removed.values[1]} — ${removed.values[2]}`);
  }
...
async function handlePhoto(message) {
  ...
  try {
    ...
    // merchant — useful when OCR misreads them or a receipt doesn't show
    // them clearly. E.g. "21/07/2026 BigBasket", "21/07/2026", or "BigBasket".
