...
async function analyzeReceipt(imageBuffer) {
  ...
  if (looksLikeTaxInvoiceFormat_(rawText)) {
    const extracted = extractTaxInvoiceFields_(rawText, rows);
...
// --- Dedicated path for structured GST tax invoices (e.g. BigBasket-style),
// which have a fixed multi-column layout very different from a simple till
// receipt: Quantity, Unit Price, Unit Taxable Value, Gross Value,
...
// always in that order, with TOTAL Value always the rightmost column. ---

function looksLikeTaxInvoiceFormat_(rawText) {
  const t = rawText.toLowerCase();
  // Individual keywords, not contiguous phrases — OCR often wraps a table
  // header across lines, splitting "HSN Code" into "HSN" ... "Code" on
  // separate lines, so requiring them stuck together was too strict.
  ...
  return hasHsn && hasGstColumn && hasTaxable;
}
...
function extractTaxInvoiceItems_(rows) {
  const items = [];
  for (const row of rows) {
    let numberEntries = [];
    row.words.forEach((w, idx) => {
    ...
    const quantity = numberEntries[0].value;
...
function extractTaxInvoiceFields_(rawText, rows) {
  const lines = rawText.split(/\r?\n/).map((l) => l.trim()).filter(Boolean);
  const merchant = lines[0] || 'Unknown';
  ...
  const items = extractTaxInvoiceItems_(rows);
