...
function extractLineItemsFromRows_(rows) {
  ...
  function nameFromRow_(row, beforeIdx) {
    ...
    for (let i = 0; i < slice.length; i++) {
      ...
      if (nameWords.length === 0 && t.length === 1) continue; // stray serial-index char (e.g. OCR misreading "3" as "З")
...
function buildInstamartName_(row, beforeIdx) {
  const nameWords = row.words
    .slice(0, beforeIdx)
    .map((w) => w.text)
    .filter((t) => t && !/^\d+\.?$/.test(t) && !/^(nos?|uqc|kgs?|ml|ltrs?|l|pcs?)$/i.test(t) && !/^[:\-%.,]+$/.test(t));
  return nameWords.join(' ').replace(/\s+/g, ' ').trim();
}

function extractInstamartItems_(rows) {
  const items = [];
  const claimedRows = new Set();

  // Pass 1: name and numbers in the same row.
  rows.forEach((row, rowIdx) => {
    const numberEntries = [];
    row.words.forEach((w, idx) => {
      if (isNumericOrPercentToken_(w.text)) numberEntries.push({ idx, value: parseNumericOrPercentToken_(w.text) });
    });
    if (numberEntries.length < 5) return; // too few numbers to plausibly be a data row

    const name = buildInstamartName_(row, numberEntries[0].idx);
    if (!name) return; // no name here — leave it for pass 2 to try pairing with a neighboring row

    claimedRows.add(rowIdx);
    items.push({
      name,
      quantity: numberEntries[0].value, // Quantity is real data here — never stripped
      rate: numberEntries.length > 1 ? numberEntries[1].value : 0, // no true per-unit Rate column; Taxable Value stands in
      amount: numberEntries[numberEntries.length - 1].value, // Total Amount is always the rightmost column
    });
  });

  // Pass 2: a pure item-name row (no numbers of its own — the name wrapped
  // onto its own line, separate from its data row) paired with the nearest
  // neighboring row that has enough numbers and no name of its own.
  rows.forEach((row, rowIdx) => {
    if (claimedRows.has(rowIdx)) return;
    const hasNumbers = row.words.some((w) => isNumericOrPercentToken_(w.text));
    if (hasNumbers) return;
    const name = buildInstamartName_(row, row.words.length);
    if (!name) return;
    ...
    for (const offset of [1, -1, 2, -2]) {
      ...
      const numberEntries = [];
      neighborRow.words.forEach((w, idx) => {
        if (isNumericOrPercentToken_(w.text)) numberEntries.push({ idx, value: parseNumericOrPercentToken_(w.text) });
      });
      if (numberEntries.length < 5) continue;
      const neighborOwnName = buildInstamartName_(neighborRow, numberEntries[0].idx);
      if (neighborOwnName) continue; // that row already has its own name — probably a different item

      claimedRows.add(rowIdx);
      claimedRows.add(neighborIdx);
      items.push({
        name,
        quantity: numberEntries[0].value,
        rate: numberEntries.length > 1 ? numberEntries[1].value : 0,
        amount: numberEntries[numberEntries.length - 1].value,
      });
      break;
    }
  });
...
function extractInstamartFields_(rawText, rows) {
  const lines = rawText.split(/\r?\n/).map((l) => l.trim()).filter(Boolean);
  const merchant = lines[0] || 'Unknown';
  ...
  const items = extractInstamartItems_(rows);
  const total = items.length ? Math.round(items.reduce((sum, it) => sum + it.amount, 0) * 100) / 100 : 0;
...
/**
 * Parses a photo caption for an optional date override and an optional
 * merchant override, e.g.:
 *   "21/07/2026"              -> date only
 *   "21/07/2026 BigBasket"    -> date + merchant
 *   "BigBasket"                -> merchant only (no date-shaped text found)
 * The date must be DD/MM/YYYY-style (see formatReceiptDate); everything
 * else in the caption, after removing the date if present, becomes the
 * merchant override.
 */
function parseCaptionOverrides(caption) {
  if (!caption) return { date: '', merchant: '' };
  const dateMatch = caption.match(/(\d{1,2}[/\-.]\d{1,2}[/\-.]\d{2,4})/);
  if (!dateMatch) return { date: '', merchant: caption.trim() };
  const date = formatReceiptDate(dateMatch[1]);
  const merchant = caption.replace(dateMatch[1], '').trim();
