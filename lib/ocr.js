...
function extractLineItemsFromRows_(rows) {
    ...
    const nameWords = row.words
      ...
      .filter(
        (t) =>
          t &&
          !/^\d+$/.test(t) &&
          !/^(hsn|gst|vat|tax|mrp|rate|qty|amt|cgst|sgst|cest|sast)$/i.test(t) &&
          !/^[:\-%.,]+$/.test(t)
      );
  ...
  // common case on a well-aligned, flat-shot receipt).
  rows.forEach((row, rowIdx) => {
    ...
    const name = nameFromRow_(row, last3[0].idx);
    if (!name || NON_ITEM_LINE.test(name)) return;

    last3.forEach((n) => (n.consumed = true));
  ...
  // Search nearby rows for an unclaimed number that validates rate*qty≈amount.
  rows.forEach((row, rowIdx) => {
    ...
    for (const offset of [-1, 1, -2, 2]) {
      ...
      const candidate = rowNumbers[neighborIdx].find(
        (n) => !n.consumed && Math.abs(n.value - expected) / expected < 0.08
      );
      if (candidate) {
        amountEntry = candidate;
        break;
      }
    }
    ...
    rateEntry.consumed = true;
    qtyEntry.consumed = true;
    amountEntry.consumed = true;
    items.push({ name, rate, quantity: qty, amount: amountEntry.value });
  });
  return items;
}
