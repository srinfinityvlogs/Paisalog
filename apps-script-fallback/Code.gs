...
function appendTransaction_(p) {
  ...
  sheet_(TRANSACTIONS_TAB).appendRow([
    ...
    p.expenseType || '',                                      // B Expense Type
    p.category || '',                                         // C Category
    p.amount || 0,                                             // D Amount
    p.merchant || '',                                          // E Merchant
    p.rate === undefined || p.rate === '' ? '' : p.rate,       // F Rate
    p.quantity === undefined || p.quantity === '' ? '' : p.quantity, // G Qty
    month,                                                      // H Month
    p.notes || '',                                              // I Notes
    p.source || '',                                             // J Source
    p.rawInput || '',                                           // K Raw Input/Raw OCR Text
  ]);
  return true;
}
...
function getLastRow_() {
  ...
  const values = sh.getRange(lastRow, 1, 1, 11).getValues()[0];
